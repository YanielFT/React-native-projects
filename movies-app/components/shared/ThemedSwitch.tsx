...
import { Platform, Pressable, SwitchProps, useColorScheme } from "react-native";
  ...
  return (
    <Pressable
      className={`flex items-center
         flex-row w-full justify-between active:opacity-80 ${className}`}
      onPress={() => onValueChange(!value)}
    >
      {text && <ThemedText type="normal">{text}</ThemedText>}
      <Switch
        trackColor={{ true: color, false: "#b7b7b7" }}
